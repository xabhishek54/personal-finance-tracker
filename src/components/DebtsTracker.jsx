...
export default function DebtsTracker() {
  ...
  const pendingLent = debtTx.filter(t => t.type === 'Lend' && !t.settled).reduce((sum, t) => sum + t.amount, 0);
  const pendingBorrowed = debtTx.filter(t => t.type === 'Borrow' && !t.settled).reduce((sum, t) => sum + t.amount, 0);
    ...
    <div className="flex flex-col gap-6 animate-[slideUp_180ms_ease-out] h-full">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="surface-card p-5 flex flex-col gap-2">
          ...
          <span className="text-2xl font-bold tabular-nums text-[var(--status-green)]">₹{pendingLent.toLocaleString()}</span>
        </div>
        <div className="surface-card p-5 flex flex-col gap-2">
          <span className="text-sm text-[var(--text-muted)] font-medium">You owe</span>
          <span className="text-2xl font-bold tabular-nums text-[var(--status-red)]">₹{pendingBorrowed.toLocaleString()}</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-[400px]">
        ...
        <div className="surface-card divide-y divide-[var(--bg-surface-lit)]">
          {debtTx.length > 0 ? debtTx.map((tx) => (
            <div key={tx.id} className={`p-4 flex items-center justify-between transition-colors ${tx.settled ? 'opacity-50' : 'hover:bg-[var(--bg-surface-lit)]'}`}>
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  tx.type === 'Lend' ? 'bg-[var(--status-green)]/10 text-[var(--status-green)]' : 'bg-[var(--status-red)]/10 text-[var(--status-red)]'
                }`}>
                  <User size={20} />
                </div>
                <div>
                  <p className="font-medium text-[var(--text-main)]">
                    {tx.type === 'Lend' ? `Lent to ${tx.recipient}` : `Borrowed from ${tx.recipient}`}
                  </p>
              ...
              <div className="flex items-center gap-4">
                <div className={`font-bold tabular-nums ${tx.type === 'Lend' ? 'text-[var(--status-green)]' : 'text-[var(--status-red)]'}`}>
                ...
                {!tx.settled ? (
                  ...
                  <span className="text-xs font-bold text-[var(--status-green)] uppercase tracking-wider px-2 py-1 bg-[var(--status-green)]/10 rounded-lg">Settled</span>
                )}
              </div>
            </div>
          )) : (
