          ...
          <div className="z-10 flex flex-col">
            ...
            {cycleTxs.length > 0 && cycleTxs.some((t) => t.type === 'Expense') && (
              <span className="text-[10px] text-[var(--status-red)] mt-1 font-medium">
                Last: - ₹
                {Number(cycleTxs.find((t) => t.type === 'Expense')?.amount || 0).toLocaleString()} (
                {cycleTxs.find((t) => t.type === 'Expense')?.category || ''})
              </span>
            )}
          </div>
          ...
          <div className="z-10 flex flex-col">
            ...
            {cycleTxs.length > 0 && cycleTxs.some((t) => t.type === 'Income') && (
              <span className="text-[10px] text-[var(--status-green)] mt-1 font-medium">
                Last: + ₹
                {Number(cycleTxs.find((t) => t.type === 'Income')?.amount || 0).toLocaleString()} (
                {cycleTxs.find((t) => t.type === 'Income')?.category || ''})
              </span>
            )}
          </div>
      ...
      <div>
        ...
        <div className="surface-card divide-y divide-[var(--bg-surface-lit)]">
          {cycleTxs.slice(0, 3).map((tx) => (
            <div
              key={tx.id}
              className="p-4 flex items-center justify-between hover:bg-[var(--bg-surface-lit)] transition-colors cursor-pointer"
            >
